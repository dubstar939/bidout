  ...
  return (
    <div className="space-y-8 animate-in fade-in zoom-in-95 duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-x-auto pb-4">
        ...
        {currentService && (
          <div className={`p-6 rounded border-2 border-dashed transition-all ${isDark ? 'bg-slate-900/20 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
            <div className="mb-6">
              ...
              <h3 className={`text-sm font-black uppercase tracking-widest mt-2 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{currentService.haulerName}</h3>
            </div>
            <div className="space-y-4">
              {metrics.map(m => (
                <div key={m.label} className="flex justify-between items-center border-b border-slate-800/50 pb-2">
                  <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{m.label}</span>
                  <span className={`text-xs font-black ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{m.format((currentService as any)[m.key])}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {selectedBids.map(bid => (
          <div key={bid.id} className={`p-6 rounded border-2 transition-all shadow-2xl relative overflow-hidden ${isDark ? 'bg-[#1e293b]/80 border-teal-500/30' : 'bg-white border-teal-600/20'}`}>
            {bid.isBestValue && (
            ...
            <div className="mb-6">
              ...
              <h3 className={`text-sm font-black uppercase tracking-widest mt-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>{bid.haulerName}</h3>
            </div>
            <div className="space-y-4">
              {metrics.map(m => {
                ...
                return (
                  <div key={m.label} className={`flex justify-between items-center border-b pb-2 ${isDark ? 'border-slate-800' : 'border-slate-100'}`}>
                    <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{m.label}</span>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-black ${isDark ? 'text-white' : 'text-slate-900'} ${isBetter ? 'text-emerald-400' : isWorse ? 'text-rose-400' : ''}`}>
                        {m.format(val)}
                      </span>
