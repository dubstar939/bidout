...
const MarketBidsSection: React.FC<MarketBidsSectionProps> = ({ 
  ...
  const getExpirationStatus = (dateStr?: string) => {
    ...
    return {
      isWithin30Days: diffDays >= 0 && diffDays <= 30,
      ...
      {prospectiveBids.length > 0 ? (
        <div className="grid grid-cols-1 gap-4">
          {prospectiveBids.map(bid => {
            const expStatus = getExpirationStatus(bid.expirationDate);
            return (
                 ...
                 <div className="p-6 flex flex-col md:flex-row justify-between md:items-center gap-6">
                    <div className="flex-grow">
                       <div className="flex flex-wrap items-center gap-3 mb-4">
                         ...
                         {expStatus?.isExpired && (
                           <span className={`border text-[9px] px-2 py-0.5 rounded-sm font-black uppercase tracking-tighter ${isDark ? 'bg-rose-500/20 text-rose-400 border-rose-500/30' : 'bg-rose-100 text-rose-800 border-rose-200'}`}>
                             Expired
                           </span>
                         )}
                         {expStatus?.isWithin30Days && (
                           <span className={`border text-[9px] px-2 py-0.5 rounded-sm font-black uppercase tracking-tighter ${isDark ? 'bg-amber-500/20 text-amber-400 border-amber-500/30' : 'bg-amber-100 text-amber-800 border-amber-200'}`}>
                             Expires in {expStatus.daysLeft} {expStatus.daysLeft === 1 ? 'day' : 'days'}
                           </span>
                         )}
                       </div>
